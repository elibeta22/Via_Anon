...
$SQL_GET_MIND = "SELECT id, date_uploaded, mind, audio_path, image_path, video_path FROM mind_image ORDER BY RAND() LIMIT 0,1";
$tmpid = 92221992;
}
...
//ROBERT TAKE A LOOK AT THIS
if($tmpid != 92221992){
$SQL_GET_COMMENTS = "SELECT id, datetime_of_comment, mind_image_pair_id, comment, user from usercomments where mind_image_pair_id = $tmpid order by id desc";
unset($_SESSION['id']);
}else{
$SQL_GET_COMMENTS = "SELECT id, datetime_of_comment, mind_image_pair_id, comment, user from usercomments where mind_image_pair_id = $id";
...
while ($row = mysqli_fetch_array($commentresult)) {
    ...
    echo "<table border='1'><tr>";
       echo "<th>$row[4]</th><td>$row[3]<div class='comment_time' align='right'>$row[1]</div></td>";
       echo '</tr>';
   echo '</table>';
